...
export class SecureRandom {
  public static DEFAULT_CHARACTER_SET =
  ...
  static generate(
    length: number = 36,
    chars: string = SecureRandom.DEFAULT_CHARACTER_SET
  ) {
  ...
  static _generate(length: number, chars: string) {
    ...
    if (!isFinite(length)) {
      throw new TypeError(
        `Argument 'length' has to be a finite integer. Got: ` + length
      );
    }
    ...
    if (charsLength > 256) {
      throw new Error(
        `Argument 'chars' should not have more than 256 characters, otherwise unpredictability ` +
          `will be broken`
      );
    }
